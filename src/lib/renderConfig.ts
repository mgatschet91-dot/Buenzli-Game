...
export interface SpritePack {
  ...
  // Path to the trees sprite sheet (separate sheet for tree/vegetation variants)
  treesSrc?: string;
  // Trees layout configuration (columns and rows for the trees sheet)
  treesCols?: number;
  treesRows?: number;
  // Trees variants: maps building type to available variants in the trees sheet
  // Each variant specifies row and column (0-indexed) in the trees sprite sheet
  treesVariants?: Record<string, { row: number; col: number }[]>;
  // Per-building-type vertical offset adjustments for TREES sprite sheet buildings
  treesVerticalOffsets?: Record<string, number>;
  // Per-building-type horizontal offset adjustments for TREES sprite sheet buildings
  treesHorizontalOffsets?: Record<string, number>;
  // Per-building-type scale adjustments for TREES sprite sheet buildings
  treesScales?: Record<string, number>;
  // Standalone building images (individual PNGs instead of sprite sheet cells)
  standaloneSrcs?: Record<string, { normal: string; construction?: string; abandoned?: string; stretchToFit?: boolean }>;
  // Per-building-type vertical offset adjustments for standalone buildings
  standaloneVerticalOffsets?: Record<string, number>;
  // Per-building-type horizontal offset adjustments for standalone buildings
  standaloneHorizontalOffsets?: Record<string, number>;
  // Per-building-type scale adjustments for standalone buildings
  standaloneScales?: Record<string, number>;
...
const SPRITE_PACK_SPRITES4: SpritePack = {
  ...
  parksBuildings: {
    ...
    woodcutter_house: { row: 3, col: 3 }, // Reuse cabin_house sprite (differentiated by "Holzfäller" label)
  ...
  parksVerticalOffsets: {
    ...
    cabin_house: -0.2,
    woodcutter_house: -0.2, // Same offset as cabin_house
    campground: -0.15,
    marina_docks_small: -0.2,  // Lowered so docks sit deeper into shoreline water
    pier_large: 0.1,  // Lowered so single pier posts sit into water
  ...
  infrastructureVariants: {
    // Water Tower levels - Row 0, columns 0-4
    water_tower: [
      { row: 0, col: 0 }, // Level 1
      { row: 0, col: 1 }, // Level 2
      { row: 0, col: 2 }, // Level 3
    ...
    // Water Reservoir levels - Row 1, columns 0-4
    water_reservoir: [
      { row: 1, col: 0 }, // Level 1
      { row: 1, col: 1 }, // Level 2
      { row: 1, col: 2 }, // Level 3
  ...
  // Trees sprite sheet configuration (tree/vegetation variants)
  treesSrc: '/assets/trees.webp',
  treesCols: 6,
  treesRows: 6,
  treesVariants: {
    // NOTE: 'tree' is NOT here - the stock tree uses the main sprite sheet as before
    // Row 0: Deciduous trees (each type = 1 fixed sprite)
    tree_oak:    [{ row: 0, col: 0 }],
    tree_maple:  [{ row: 0, col: 1 }],
    tree_birch:  [{ row: 0, col: 2 }],
    tree_willow: [{ row: 0, col: 4 }],
    // Row 1: Evergreen trees
    tree_pine:   [{ row: 1, col: 0 }],
    tree_spruce: [{ row: 1, col: 1 }],
    tree_fir:    [{ row: 1, col: 2 }],
    tree_cedar:  [{ row: 1, col: 3 }],
    // Row 2: Tropical trees
    tree_palm:    [{ row: 2, col: 0 }],
    tree_bamboo:  [{ row: 2, col: 2 }],
    tree_coconut: [{ row: 2, col: 3 }],
    // Row 3: Flowering trees
    tree_cherry:    [{ row: 3, col: 0 }],
    tree_magnolia:  [{ row: 3, col: 1 }],
    tree_jacaranda: [{ row: 3, col: 3 }],
    tree_wisteria:  [{ row: 3, col: 4 }],
    // Row 4: Bushes & topiary
    bush_hedge:     [{ row: 4, col: 0 }],
    bush_flowering: [{ row: 4, col: 1 }],
    topiary_ball:   [{ row: 4, col: 2 }],
    topiary_spiral: [{ row: 4, col: 3 }],
    // Row 5: Flowers & ground cover
    flower_bed:     [{ row: 5, col: 0 }],
    flower_planter: [{ row: 5, col: 1 }],
  },
  treesVerticalOffsets: {
    tree_oak: -0.35, tree_maple: -0.35, tree_birch: -0.35, tree_willow: -0.40,
    tree_pine: -0.40, tree_spruce: -0.40, tree_fir: -0.40, tree_cedar: -0.40,
    tree_palm: -0.35, tree_bamboo: -0.35, tree_coconut: -0.35,
    tree_cherry: -0.30, tree_magnolia: -0.30, tree_jacaranda: -0.30, tree_wisteria: -0.30,
    bush_hedge: -0.20, bush_flowering: -0.20, topiary_ball: -0.15, topiary_spiral: -0.15,
    flower_bed: -0.10, flower_planter: -0.15,
  },
  treesHorizontalOffsets: {},
  treesScales: {
    tree_oak: 0.75, tree_maple: 0.75, tree_birch: 0.70, tree_willow: 0.80,
    tree_pine: 0.75, tree_spruce: 0.75, tree_fir: 0.75, tree_cedar: 0.75,
    tree_palm: 0.75, tree_bamboo: 0.65, tree_coconut: 0.75,
    tree_cherry: 0.70, tree_magnolia: 0.70, tree_jacaranda: 0.70, tree_wisteria: 0.70,
    bush_hedge: 0.80, bush_flowering: 0.80, topiary_ball: 0.55, topiary_spiral: 0.60,
    flower_bed: 0.70, flower_planter: 0.80,
  },
  ...
  standaloneSrcs: {
    bank_house: {
      normal: '/assets/buildings/bank_normal.png',
      construction: '/assets/buildings/bank_normal.png',
      abandoned: '/assets/buildings/bank_damaged.png',
    },
    bus_stop: {
      normal: '/assets/buildings/bus_stop.png',
    },
    bus_station: {
      normal: '/assets/buildings/bus_station.png',
    },
    solar_panel: {
      normal: '/assets/buildings/solar-panal.png',
    },
    fcbasel_stadium: {
      normal: '/assets/buildings/fcbasel.png',
    },
    st_ursen_kathedrale: {
      normal: '/assets/buildings/stursenkathedrale.png',
    },
    primetower: {
  ...
  buildingToSprite: {
    ...
    tree: 'tree',
    // Trees from trees.webp (each maps to 'tree' base sprite as fallback)
    tree_oak: 'tree', tree_maple: 'tree', tree_birch: 'tree', tree_willow: 'tree',
    tree_pine: 'tree', tree_spruce: 'tree', tree_fir: 'tree', tree_cedar: 'tree',
    tree_palm: 'tree', tree_bamboo: 'tree', tree_coconut: 'tree',
    tree_cherry: 'tree', tree_magnolia: 'tree', tree_jacaranda: 'tree', tree_wisteria: 'tree',
    bush_hedge: 'tree', bush_flowering: 'tree', topiary_ball: 'tree', topiary_spiral: 'tree',
    flower_bed: 'tree', flower_planter: 'tree',
    water: 'water',
    subway_station: 'subway_station',
    // bus_stop uses custom placeholder renderer (drawBusStopPlaceholder) — no sprite mapping needed
