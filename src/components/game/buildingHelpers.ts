...
const PARK_BUILDINGS_SET = new Set<BuildingType>([
  ...
  'marina_docks_small', 'roller_coaster_small', 'mountain_lodge', 'playground_large', 'mountain_trailhead', 'woodcutter_house'
]);
...
const ALL_PARK_TYPES = new Set<BuildingType>([
  ...
  'mountain_lodge', 'mountain_trailhead', 'woodcutter_house',
...
export function useBuildingHelpers(grid: Tile[][], gridSize: number) {
  ...
  const tileMetadataMap = useMemo(() => {
    ...
    // Guard: Leeres Grid → leere Map zurückgeben
    if (!grid || grid.length === 0 || gridSize === 0) return map;
