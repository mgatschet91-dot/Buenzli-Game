...
// ── Debug Cloud Override (module-level, set from DebugPanel) ──
export interface DebugCloudOverride {
  type: CloudType | 'all' | null;
  density: number; // 0.2 = sparse, 1 = normal, 3 = dense
  clearExisting?: boolean;
}

let _debugCloudOverride: DebugCloudOverride | null = null;
let _debugCloudDirty = false;

export function setDebugCloudOverride(override: DebugCloudOverride | null): void {
  _debugCloudOverride = override;
  _debugCloudDirty = true;
}

export function getDebugCloudOverride(): DebugCloudOverride | null {
  return _debugCloudOverride;
}

// ── Dynamic wind direction from server weather ──
// Meteorological degrees (0=N, 90=E, 180=S, 270=W) = direction wind blows FROM
let _serverWindAngle: number = CLOUD_WIND_ANGLE;

/** Convert meteorological wind direction (degrees, where wind comes FROM) to canvas radians */
function meteoToCanvasAngle(degrees: number): number {
  const movementCompass = (degrees + 180) % 360;
  return (movementCompass - 90) * Math.PI / 180;
}

export function setServerWindDirection(meteoDegrees: number): void {
  _serverWindAngle = meteoToCanvasAngle(meteoDegrees);
}

function getWindAngle(): number {
  return _serverWindAngle;
}
import {
  FIREWORK_BUILDINGS,
  FIREWORK_COLORS,
...
export function useEffectsSystems(
  ...
  const pickCloudType = useCallback((currentHour: number): CloudType => {
    const dbg = _debugCloudOverride;
    if (dbg?.type && dbg.type !== 'all') {
      return dbg.type;
    }
    if (dbg?.type === 'all') {
      return CLOUD_TYPES_ORDERED[Math.floor(Math.random() * CLOUD_TYPES_ORDERED.length)];
    }
  ...
  const generateCloudPuffs = useCallback((cloudType: CloudType): CloudPuff[] => {
    ...
    switch (cloudType) {
      ...
      case 'cumulonimbus': {
        // Massive towering thundercloud: very dark/wide base, bright anvil top
        // Wide dark base (3-4 large overlapping puffs)
        puffs.push({ offsetX: jitter(-30, 8), offsetY: jitter(35, 6), size: jitter(65, 12), opacity: 0.95, portion: 'base' });
        puffs.push({ offsetX: jitter(0, 8),   offsetY: jitter(32, 6), size: jitter(70, 12), opacity: 0.95, portion: 'base' });
        puffs.push({ offsetX: jitter(28, 8),  offsetY: jitter(34, 6), size: jitter(60, 12), opacity: 0.9,  portion: 'base' });
        puffs.push({ offsetX: jitter(-12, 6), offsetY: jitter(28, 5), size: jitter(55, 10), opacity: 0.9,  portion: 'base' });
        // Dark middle transition (3 medium puffs)
        puffs.push({ offsetX: jitter(-20, 7), offsetY: jitter(12, 5), size: jitter(50, 10), opacity: 0.85, portion: 'base' });
        puffs.push({ offsetX: jitter(8, 7),   offsetY: jitter(8, 5),  size: jitter(48, 10), opacity: 0.85, portion: 'base' });
        puffs.push({ offsetX: jitter(25, 6),  offsetY: jitter(14, 5), size: jitter(45, 8),  opacity: 0.8,  portion: 'base' });
        // Upper tower (brighter transition)
        puffs.push({ offsetX: jitter(-10, 8), offsetY: jitter(-6, 5), size: jitter(44, 8), opacity: 0.85, portion: 'top' });
        puffs.push({ offsetX: jitter(12, 8),  offsetY: jitter(-10, 5), size: jitter(40, 8), opacity: 0.85, portion: 'top' });
        // Bright anvil top (spreads wider)
        puffs.push({ offsetX: jitter(-25, 10), offsetY: jitter(-22, 5), size: jitter(42, 8), opacity: 0.9, portion: 'top' });
        puffs.push({ offsetX: jitter(5, 10),   offsetY: jitter(-26, 5), size: jitter(46, 8), opacity: 0.9, portion: 'top' });
        puffs.push({ offsetX: jitter(30, 10),  offsetY: jitter(-20, 5), size: jitter(38, 6), opacity: 0.85, portion: 'top' });
        break;
      }
  ...
  // spawnInView: true = spawn randomly INSIDE the viewport (for debug/instant fill)
  const spawnCloud = useCallback((currentHour: number, opts?: { position?: { x: number; y: number }; cloudType?: CloudType; spawnInView?: boolean }) => {
    ...
    const currentWindAngle = getWindAngle();
    const windX = Math.cos(currentWindAngle);
    const windY = Math.sin(currentWindAngle);
    ...
    } else if (opts?.spawnInView) {
      // Spawn randomly within the visible viewport
  ...
  const updateClouds = useCallback((delta: number, currentHour: number) => {
    ...
    // Debug override: clear existing clouds and instantly fill viewport
    if (_debugCloudDirty) {
      _debugCloudDirty = false;
      if (_debugCloudOverride?.clearExisting) {
        cloudsRef.current = [];
        cloudSpawnTimerRef.current = 0;
        // Instantly spawn a batch inside the viewport so they're visible immediately
        const fillCount = Math.round((isMobile ? 6 : 12) * (_debugCloudOverride.density ?? 1));
        for (let i = 0; i < fillCount; i++) {
          spawnCloud(currentHour, { spawnInView: true });
        }
      }
    }

    const dbg = _debugCloudOverride;
    const densityMult = dbg?.density ?? 1;
    
    const baseMaxClouds = isMobile ? CLOUD_MAX_COUNT_MOBILE : CLOUD_MAX_COUNT;
    const maxClouds = Math.round(baseMaxClouds * densityMult);
    const baseSpawnInterval = isMobile ? CLOUD_SPAWN_INTERVAL_MOBILE : CLOUD_SPAWN_INTERVAL;
    const spawnInterval = baseSpawnInterval / Math.max(0.2, densityMult);
