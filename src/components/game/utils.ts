...
// Check if a tile is an autobahn tile (separate from road - no mixing)
export function isAutobahnTile(gridData: Tile[][], gridSizeValue: number, x: number, y: number): boolean {
  if (x < 0 || y < 0 || x >= gridSizeValue || y >= gridSizeValue) return false;
  return gridData[y][x].building.type === 'autobahn';
}

// Check if a tile is drivable (road OR autobahn) - used for spawn counting
export function isDrivableTile(gridData: Tile[][], gridSizeValue: number, x: number, y: number): boolean {
  return isRoadTile(gridData, gridSizeValue, x, y) || isAutobahnTile(gridData, gridSizeValue, x, y);
}
...
// Get direction options for autobahn (only connects to other autobahn tiles)
export function getAutobahnDirectionOptions(gridData: Tile[][], gridSizeValue: number, x: number, y: number): CarDirection[] {
  const options: CarDirection[] = [];
  if (isAutobahnTile(gridData, gridSizeValue, x - 1, y)) options.push('north');
  if (isAutobahnTile(gridData, gridSizeValue, x, y - 1)) options.push('east');
  if (isAutobahnTile(gridData, gridSizeValue, x + 1, y)) options.push('south');
  if (isAutobahnTile(gridData, gridSizeValue, x, y + 1)) options.push('west');
  return options;
}
...
export function pickNextDirection(
  ...
  // Autobahn: never reverse, follow the road, despawn at dead ends
  const currentTile = gridData[y]?.[x];
  if (currentTile?.building.type === 'autobahn') {
    const options = getAutobahnDirectionOptions(gridData, gridSizeValue, x, y);
    if (options.length === 0) return null;
    // Never reverse on autobahn
    const incoming = getOppositeDirection(previousDirection);
    const filtered = options.filter(dir => dir !== incoming);
    if (filtered.length === 0) return null; // Dead end: despawn
    const storedDir = currentTile.building.autobahnDirection as CarDirection | undefined;
    if (storedDir && filtered.includes(storedDir)) return storedDir;
    if (filtered.includes(previousDirection)) return previousDirection;
