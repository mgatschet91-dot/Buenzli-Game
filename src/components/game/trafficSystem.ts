...
/**
 * Analyze autobahn tile connectivity (separate network from roads).
 * Returns a MergedRoadInfo-compatible object for rendering.
 */
export function analyzeAutobahn(
  grid: Tile[][],
  gridSize: number,
  x: number,
  y: number
): MergedRoadInfo {
  const defaultInfo: MergedRoadInfo = {
    type: 'single',
    orientation: 'intersection',
    laneCount: 2,
    hasMedian: false,
  ...
  if (x < 0 || y < 0 || x >= gridSize || y >= gridSize) return defaultInfo;
  if (grid[y]?.[x]?.building.type !== 'autobahn') return defaultInfo;

  const hasN = x > 0 && grid[y]?.[x - 1]?.building.type === 'autobahn';
  const hasE = y > 0 && grid[y - 1]?.[x]?.building.type === 'autobahn';
  const hasS = x < gridSize - 1 && grid[y]?.[x + 1]?.building.type === 'autobahn';
  const hasW = y < gridSize - 1 && grid[y + 1]?.[x]?.building.type === 'autobahn';

  const isNS = hasN || hasS;
  const isEW = hasE || hasW;
  let orientation: RoadOrientation = 'intersection';
  if (isNS && !isEW) orientation = 'ns';
  else if (isEW && !isNS) orientation = 'ew';

  return { ...defaultInfo, orientation };
}
