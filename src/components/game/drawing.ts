...
import { Tile, ZoneType, TilePaintColor } from '@/types/game';
import { TILE_WIDTH, TILE_HEIGHT, HEIGHT_STEP } from './types';
...
export const BAUZONE_COLORS = {
  fill: 'rgba(0, 200, 220, 0.12)',
...
/** Custom paint color schemes for tile painting */
export const PAINT_COLORS: Record<TilePaintColor, TileColorScheme> = {
  green: {
    top: '#4a7c3f',
    left: '#3d6634',
    right: '#5a8f4f',
...
/** Elevation height in pixels per elevation level (re-exported for convenience) */
export const ELEVATION_HEIGHT = HEIGHT_STEP;
...
/**
 * Get the effective tile colors considering zone and paint color.
 */
export function getEffectiveTileColors(tile: Tile): TileColorScheme {
  if (tile.paintColor) {
    return PAINT_COLORS[tile.paintColor];
  }
  return ZONE_COLORS[tile.zone];
}

/**
 * Draw cliff/side faces for a tile that is higher than its neighbors.
 * This is called BEFORE the top face to create proper 3D cliffs.
 * 
 * @param neighborElevations - elevation of neighboring tiles { south, west }
 *   south = neighbor at grid (x+1, y) - appears to the RIGHT in isometric
 *   west  = neighbor at grid (x, y+1) - appears to the LEFT in isometric
 */
export function drawCliffSides(
  ctx: CanvasRenderingContext2D,
  screenX: number,
  screenY: number,
  tile: Tile,
  neighborElevations: { south: number; west: number },
  currentZoom: number
): void {
  const elevation = tile.elevation || 0;
  if (elevation <= 0) return;
  
  const colors = getEffectiveTileColors(tile);
  const w = TILE_WIDTH;
  const h = TILE_HEIGHT;
  
  // The tile is drawn at screenY which is already adjusted for this tile's elevation.
  // We need to draw cliff faces that extend DOWN from the current tile to where
  // the neighbor's surface would be.
  
  // South neighbor (appears at bottom-right in isometric) - RIGHT cliff face
  const dzSouth = elevation - neighborElevations.south;
  if (dzSouth > 0) {
    const cliffH = dzSouth * HEIGHT_STEP;
    ctx.fillStyle = colors.right;
    ctx.beginPath();
    ctx.moveTo(screenX + w, screenY + h / 2);         // right corner of this tile
    ctx.lineTo(screenX + w / 2, screenY + h);          // bottom corner of this tile
    ctx.lineTo(screenX + w / 2, screenY + h + cliffH); // bottom corner + cliff height
    ctx.lineTo(screenX + w, screenY + h / 2 + cliffH); // right corner + cliff height
    ctx.closePath();
    ctx.fill();
    if (currentZoom >= 0.6) {
      ctx.strokeStyle = colors.stroke;
  ...
  // West neighbor (appears at bottom-left in isometric) - LEFT cliff face
  const dzWest = elevation - neighborElevations.west;
  if (dzWest > 0) {
    const cliffH = dzWest * HEIGHT_STEP;
    ctx.fillStyle = colors.left;
    ctx.beginPath();
    ctx.moveTo(screenX, screenY + h / 2);              // left corner of this tile
    ctx.lineTo(screenX + w / 2, screenY + h);           // bottom corner of this tile
    ctx.lineTo(screenX + w / 2, screenY + h + cliffH);  // bottom corner + cliff height
    ctx.lineTo(screenX, screenY + h / 2 + cliffH);      // left corner + cliff height
    ctx.closePath();
    ctx.fill();
    if (currentZoom >= 0.6) {
      ctx.strokeStyle = colors.stroke;
 ...
 * Colors are determined by the tile's zone and paint color.
 * NOTE: Elevation Y-offset is now handled by the caller (CanvasIsometricGrid).
 * This function just draws the top diamond + zone borders.
 */
export function drawGreenBaseTile(
  ...
  if (elevation >= 4 && currentZoom >= 0.4) {
    ...
    const cy = corners.top.y + (corners.bottom.y - corners.top.y) * 0.35;
    const peakSize = TILE_WIDTH * 0.15;
