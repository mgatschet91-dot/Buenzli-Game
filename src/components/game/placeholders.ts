...
export const PLACEHOLDER_COLORS: Record<string, PlaceholderColor> = {
  ...
  bus_stop: { top: '#fbbf24', left: '#d97706', right: '#fcd34d', height: 0.3 },
  // Special - golds
...
export function drawPlaceholderBuilding(
  ...
  // Custom bus stop rendering
  if (buildingType === 'bus_stop') {
    drawBusStopPlaceholder(ctx, x, y, tileWidth, tileHeight);
    return;
  }
...
/**
 * Draw a bus stop: small yellow shelter with PostAuto-style sign pole.
 * Rendered as isometric: flat platform + shelter roof + sign pole with "H" sign.
 */
function drawBusStopPlaceholder(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  tileWidth: number,
  ...
  const h = tileHeight;
  const cx = x + w / 2;
  const scale = w / 64; // Normalize to 64px base tile

  // --- Flat concrete platform (isometric diamond, very thin) ---
  const platH = h * 0.08;
  ...
  ctx.beginPath();
  ctx.moveTo(cx, y + h * 0.3 - platH);
  ctx.lineTo(x + w * 0.85, y + h * 0.65 - platH);
  ctx.lineTo(cx, y + h - platH);
  ctx.lineTo(x + w * 0.15, y + h * 0.65 - platH);
  ctx.closePath();
  ctx.fill();

  // --- Small shelter / Wartehäuschen (right half of platform) ---
  const shelterBaseY = y + h * 0.65 - platH;
  const shelterH = 18 * scale;

  // Shelter back wall (left face)
  ctx.fillStyle = '#475569'; // slate-600
  ctx.beginPath();
  ctx.moveTo(cx - 2 * scale, shelterBaseY - 2 * scale);
  ctx.lineTo(cx + 10 * scale, shelterBaseY + 4 * scale);
  ctx.lineTo(cx + 10 * scale, shelterBaseY + 4 * scale - shelterH);
  ctx.lineTo(cx - 2 * scale, shelterBaseY - 2 * scale - shelterH);
  ctx.closePath();
  ctx.fill();

  // Shelter side wall (right face)
  ctx.fillStyle = '#64748b'; // slate-500
  ctx.beginPath();
  ctx.moveTo(cx + 10 * scale, shelterBaseY + 4 * scale);
  ctx.lineTo(cx + 18 * scale, shelterBaseY);
  ctx.lineTo(cx + 18 * scale, shelterBaseY - shelterH);
  ctx.lineTo(cx + 10 * scale, shelterBaseY + 4 * scale - shelterH);
  ctx.closePath();
  ctx.fill();

  // Shelter roof (PostAuto yellow)
  const roofOverhang = 3 * scale;
  ctx.fillStyle = '#f59e0b'; // amber-500
  ctx.beginPath();
  ctx.moveTo(cx - 2 * scale - roofOverhang, shelterBaseY - 2 * scale - shelterH - 2 * scale);
  ctx.lineTo(cx + 10 * scale + roofOverhang / 2, shelterBaseY + 4 * scale - shelterH - 2 * scale);
  ctx.lineTo(cx + 18 * scale + roofOverhang, shelterBaseY - shelterH - 2 * scale);
  ctx.lineTo(cx - 2 * scale + roofOverhang / 2 - roofOverhang, shelterBaseY - 2 * scale - shelterH - 4 * scale);
  ctx.closePath();
  ctx.fill();
  // Roof edge
  ...
  const poleBaseY = shelterBaseY - 6 * scale;
  const poleTopY = poleBaseY - 26 * scale;
  ...
  // Sign (yellow rectangle with "H")
  const signW = 8 * scale;
  const signH = 10 * scale;
  const signX = poleX - signW / 2;
  const signY = poleTopY;
  ...
  ctx.fillText('H', poleX, signY + signH / 2);

  // --- Bench (small line on platform) ---
  ctx.strokeStyle = '#78716c'; // stone-500
  ctx.lineWidth = Math.max(1, 1.5 * scale);
  ctx.beginPath();
  ctx.moveTo(cx + 2 * scale, shelterBaseY - 1 * scale);
  ctx.lineTo(cx + 12 * scale, shelterBaseY + 2 * scale);
  ctx.stroke();
}
