...
const PARK_TYPES = new Set<BuildingType>([
  ...
  'community_garden', 'pond_park', 'park_gate', 'mountain_lodge', 'mountain_trailhead', 'woodcutter_house'
]);
...
/**
 * Find all stations of a specific type (fire, police, or hospital)
 */
export function findStations(
  grid: Tile[][],
  gridSize: number,
  type: 'fire_station' | 'police_station' | 'hospital'
): { x: number; y: number }[] {
  if (!grid || gridSize <= 0) return [];
...
export function findBusStops(
  grid: Tile[][],
  gridSize: number
): { x: number; y: number; explicit: boolean }[] {
  if (!grid || gridSize <= 0) return [];

  const stops: { x: number; y: number; explicit: boolean }[] = [];
  for (let y = 0; y < gridSize; y++) {
    for (let x = 0; x < gridSize; x++) {
      const btype = grid[y][x].building.type;
      if (btype === 'bus_stop') {
        stops.push({ x, y, explicit: true });
      } else if (BUS_STOP_TYPES.has(btype)) {
        stops.push({ x, y, explicit: false });
      }
    }
  }
  return stops;
