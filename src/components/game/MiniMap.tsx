...
const MINIMAP_EXPANDED_STORAGE_KEY = 'isocity_minimap_expanded';
...
export const MiniMap = React.memo(function MiniMap({ onNavigate, viewport }: MiniMapProps) {
  ...
  const [hasLoadedExpandedPref, setHasLoadedExpandedPref] = useState(false);
  ...
  useEffect(() => {
    ...
    const updateMiniMapSize = () => {
      ...
      const nextSize = Math.max(120, Math.min(220, Math.round(shortestSide * 0.2)));
      setMiniMapSize(nextSize);
    };

    updateMiniMapSize();
    window.addEventListener('resize', updateMiniMapSize);
    return () => window.removeEventListener('resize', updateMiniMapSize);
  }, []);

  useEffect(() => {
    if (typeof window === 'undefined') return;
    const saved = window.localStorage.getItem(MINIMAP_EXPANDED_STORAGE_KEY);
    if (saved === 'true') {
      setIsExpanded(true);
    } else if (saved === 'false') {
      setIsExpanded(false);
    }
    setHasLoadedExpandedPref(true);
  }, []);

  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (!hasLoadedExpandedPref) return;
    window.localStorage.setItem(MINIMAP_EXPANDED_STORAGE_KEY, String(isExpanded));
  }, [isExpanded, hasLoadedExpandedPref]);
  ...
  }, [grid, gridSize, viewport, tick, serviceBuildings, parkBuildings, miniMapSize, isExpanded]);
  ...
  }, [onNavigate, gridSize, miniMapSize]);
  ...
  if (!isExpanded) {
    return (
      <Card className="fixed bottom-4 right-4 p-1.5 shadow-xl bg-card/85 backdrop-blur-md border-border/70 z-20">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => setIsExpanded(true)}
          className="h-8 w-8 p-0"
          title={m(msg('Minimap einblenden'))}
        >
          <ChevronUp size={16} />
        </Button>
      </Card>
  ...
  return (
    <Card className="fixed bottom-4 right-4 p-2.5 sm:p-3 shadow-xl bg-card/85 backdrop-blur-md border-border/70 z-20">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground font-semibold">
          {m(MINIMAP_LABEL)}
        </div>
        <div className="flex items-center gap-1">
          <div className="text-[9px] text-muted-foreground/80">{miniMapSize}px</div>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setIsExpanded(false)}
            className="h-6 w-6 p-0"
            title={m(msg('Minimap einklappen'))}
          >
            <ChevronDown size={14} />
          </Button>
        </div>
      ...
      <canvas
        ...
        width={miniMapSize}
        height={miniMapSize}
        className="block rounded-lg border border-border/60 cursor-pointer select-none shadow-inner"
      ...
      <div className="mt-2 grid grid-cols-4 gap-1 text-[8px]">
        ...
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 bg-amber-500 rounded-[2px]" />
