...
export interface RoadDrawingOptions {
  ...
  // Autobahn customization
  asphaltColor?: string;
  skipTrafficFeatures?: boolean;
  centerLineColor?: string;
  guardrailStyle?: boolean;
}

/** Options for autobahn drawing */
export interface AutobahnDrawingOptions {
  hasAutobahn: (gridX: number, gridY: number) => boolean;
  getAutobahnMergeInfo: (gridX: number, gridY: number) => MergedRoadInfo;
  isMobile: boolean;
  isPanning: boolean;
  isPinchZooming: boolean;
}
...
export function drawRoad(
  ...
  const drawSidewalkEdge = (
    ...
    // Guardrail style: draw 3D steel guardrails instead of flat sidewalks
    if (options.guardrailStyle && edgeDir) {
      // Only draw guardrails on edges perpendicular to road direction
      const isNS = north || south;
      const isEW = east || west;
      const isIsolated = !north && !east && !south && !west;

      // NS roads: guardrails on east/west edges only
      // EW roads: guardrails on north/south edges only
      // Isolated: no guardrails
      const shouldDraw =
        isIsolated ? false :
        (isNS && !isEW) ? (edgeDir === 'east' || edgeDir === 'west') :
        (isEW && !isNS) ? (edgeDir === 'north' || edgeDir === 'south') :
        true; // intersection or mixed: draw on all edges
      ...
      ctx.beginPath();
      ctx.moveTo(actualStartX, actualStartY - wallH);
      ctx.lineTo(actualEndX, actualEndY - wallH);
      ctx.lineTo(actualEndX + inwardDx * swWidth, actualEndY + inwardDy * swWidth - wallH);
      ctx.lineTo(actualStartX + inwardDx * swWidth, actualStartY + inwardDy * swWidth - wallH);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.lineTo(actualEndX, actualEndY - wallH);
      ctx.lineTo(actualStartX, actualStartY - wallH);
      ctx.closePath();
      ctx.fill();
  ...
  if (showSidewalks && !north && isOuterEdge('north')) {
    drawSidewalkEdge(leftCorner.x, leftCorner.y, topCorner.x, topCorner.y, 0.707, 0.707, !west && isOuterEdge('west'), !east && isOuterEdge('east'), 'north');
  }
  if (showSidewalks && !east && isOuterEdge('east')) {
    drawSidewalkEdge(topCorner.x, topCorner.y, rightCorner.x, rightCorner.y, -0.707, 0.707, !north && isOuterEdge('north'), !south && isOuterEdge('south'), 'east');
  }
  if (showSidewalks && !south && isOuterEdge('south')) {
    drawSidewalkEdge(rightCorner.x, rightCorner.y, bottomCorner.x, bottomCorner.y, -0.707, -0.707, !east && isOuterEdge('east'), !west && isOuterEdge('west'), 'south');
  }
  if (showSidewalks && !west && isOuterEdge('west')) {
    drawSidewalkEdge(bottomCorner.x, bottomCorner.y, leftCorner.x, leftCorner.y, 0.707, -0.707, !south && isOuterEdge('south'), !north && isOuterEdge('north'), 'west');
  }
  ...
  // Skip corner pieces when guardrailStyle is true (autobahn has no sidewalk corners)
  if (showSidewalks && !options.guardrailStyle && !north && !east && isOuterEdge('north') && isOuterEdge('east')) {
  ...
  if (showSidewalks && !options.guardrailStyle && !east && !south && isOuterEdge('east') && isOuterEdge('south')) {
  ...
  if (showSidewalks && !options.guardrailStyle && !south && !west && isOuterEdge('south') && isOuterEdge('west')) {
  ...
  if (showSidewalks && !options.guardrailStyle && !west && !north && isOuterEdge('west') && isOuterEdge('north')) {
  ...
  // DRAW ROAD SURFACE
  // ============================================
  // Use different asphalt color for highways (or custom override for autobahn)
  ctx.fillStyle = options.asphaltColor ? options.asphaltColor :
                  mergeInfo.type === 'highway' ? '#3d3d3d' :
  ...
  // Skip interior corners when guardrailStyle is true (autobahn has no sidewalk corners)
  if (showSidewalks && !options.guardrailStyle) {
    ctx.fillStyle = sidewalkColor;
  ...
  if (currentZoom >= LANE_MARKINGS_MIN_ZOOM) {
    ...
    if (!thisIsIntersection) {
      ctx.strokeStyle = options.centerLineColor || ROAD_COLORS.CENTER_LINE;
      ctx.lineWidth = 0.8;
    ...
    // ============================================
    if (!options.skipTrafficFeatures) {
      drawCrosswalks({
        ctx,
        x,
        y,
    ...
    if (!options.skipTrafficFeatures && isIntersection && currentZoom >= TRAFFIC_LIGHT_MIN_ZOOM && !skipTrafficLights) {
...
/**
 * Draw an autobahn tile - uses drawRoad with dark asphalt, no traffic features,
 * white center lines, and 3D steel guardrails instead of sidewalks.
 */
export function drawAutobahn(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  gridX: number,
  gridY: number,
  currentZoom: number,
  options: AutobahnDrawingOptions
): void {
  drawRoad(ctx, x, y, gridX, gridY, currentZoom, {
    hasRoad: options.hasAutobahn,
    getMergeInfo: options.getAutobahnMergeInfo,
    isMobile: options.isMobile,
    isPanning: options.isPanning,
    isPinchZooming: options.isPinchZooming,
    trafficLightTimer: 0,
    asphaltColor: '#1a1a1a',
    skipTrafficFeatures: true,
...
/**
 * Create a cached autobahn merge info getter with proper invalidation
 */
export function createAutobahnMergeInfoCache(
  grid: Tile[][],
  gridSize: number,
  cacheRef: React.MutableRefObject<Map<string, MergedRoadInfo>>,
  cacheVersionRef: React.MutableRefObject<number>,
  ...
  return (gx: number, gy: number): MergedRoadInfo => {
    ...
    let info = cacheRef.current.get(key);
    if (!info) {
      info = analyzeAutobahn(grid, gridSize, gx, gy);
      cacheRef.current.set(key, info);
    }
    return info;
  };
