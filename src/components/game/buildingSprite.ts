...
export interface SpriteSourceResult {
  ...
  variantType: 'normal' | 'construction' | 'abandoned' | 'parks' | 'parksConstruction' | 'dense' | 'modern' | 'farm' | 'shop' | 'station' | 'services' | 'infrastructure' | 'mansion' | 'trees' | 'standalone' | 'standaloneConstruction' | 'standaloneAbandoned';
...
export function selectSpriteSource(
  ...
  mansionVariantOverride?: { row: number; col: number } | null
): SpriteSourceResult {
  ...
  const standaloneConfig = activePack.standaloneSrcs && activePack.standaloneSrcs[buildingType];
  if (standaloneConfig) {
    if (isUnderConstruction && standaloneConfig.construction) {
      return {
        source: standaloneConfig.construction,
        variantType: 'standaloneConstruction',
        variant: null,
      };
    }
    if (isAbandoned && standaloneConfig.abandoned) {
      return {
        source: standaloneConfig.abandoned,
        variantType: 'standaloneAbandoned',
        variant: null,
      };
    }
    return {
      source: standaloneConfig.normal,
  ...
  // Trees variants (tree/vegetation types from trees sprite sheet)
  if (activePack.treesSrc && activePack.treesVariants && activePack.treesVariants[buildingType]) {
    const variants = activePack.treesVariants[buildingType];
    if (variants.length > 0) {
      // For existing 'tree' type: deterministic random variant for visual variety
      // For specific types (tree_deciduous, etc.): also random within their category
      const variantIndex = (tileX * 31 + tileY * 17) % variants.length;
      return {
        source: activePack.treesSrc,
        variantType: 'trees',
        variant: variants[variantIndex],
      };
    }
  ...
  if (activePack.mansionsSrc && activePack.mansionsVariants && activePack.mansionsVariants[buildingType]) {
    // If a specific player-chosen variant is provided, always use it
    if (mansionVariantOverride != null) {
      return {
        source: activePack.mansionsSrc,
        variantType: 'mansion',
        variant: mansionVariantOverride,
      };
    }
...
export function calculateSpriteCoords(
  ...
  // Standalone buildings use the entire image
  if (variantType === 'standalone' || variantType === 'standaloneConstruction' || variantType === 'standaloneAbandoned') {
    return {
      sx: 0,
      sy: 0,
      sw: sheetWidth,
  ...
  if ((variantType === 'parks' || variantType === 'parksConstruction') && variant) {
    ...
    if (buildingType === 'marina_docks_small') {
      ...
      sourceY += tileHeight * 0.1;
    } else if (buildingType === 'woodcutter_house') {
      sourceY += tileHeight * 0.1;
  ...
  // Trees variants
  if (variantType === 'trees' && variant) {
    const treesCols = activePack.treesCols || 6;
    const treesRows = activePack.treesRows || 6;
    const tileWidth = Math.floor(sheetWidth / treesCols);
    const tileHeight = Math.floor(sheetHeight / treesRows);
  ...
  if (variantType === 'mansion' && variant) {
    ...
    const mansionsRows = activePack.mansionsRows || 6;
...
export function calculateSpriteScale(
  ...
  if (variantType === 'trees' && activePack.treesScales && buildingType in activePack.treesScales) {
    scaleMultiplier *= activePack.treesScales[buildingType];
  }
  ...
  if ((variantType === 'standalone' || variantType === 'standaloneConstruction' || variantType === 'standaloneAbandoned') &&
      activePack.standaloneScales && buildingType in activePack.standaloneScales) {
    scaleMultiplier *= activePack.standaloneScales[buildingType];
  }
...
export function calculateSpriteOffsets(
  ...
  } else if (variantType === 'trees' && activePack.treesVerticalOffsets &&
             buildingType in activePack.treesVerticalOffsets) {
    verticalOffset = activePack.treesVerticalOffsets[buildingType];
  ...
  } else if ((variantType === 'standalone' || variantType === 'standaloneConstruction' || variantType === 'standaloneAbandoned') &&
             activePack.standaloneVerticalOffsets && buildingType in activePack.standaloneVerticalOffsets) {
    verticalOffset = activePack.standaloneVerticalOffsets[buildingType];
  ...
  if (variantType === 'trees' && activePack.treesHorizontalOffsets && 
      buildingType in activePack.treesHorizontalOffsets) {
    horizontalOffset = activePack.treesHorizontalOffsets[buildingType];
  }

  if ((variantType === 'standalone' || variantType === 'standaloneConstruction' || variantType === 'standaloneAbandoned') &&
      activePack.standaloneHorizontalOffsets && buildingType in activePack.standaloneHorizontalOffsets) {
    horizontalOffset = activePack.standaloneHorizontalOffsets[buildingType];
  }
...
export function getSpriteRenderInfo(
  ...
  // stretchToFit: ignore image aspect ratio, stretch to match the isometric footprint slot.
  // destHeight = footprintN * TILE_HEIGHT * 2.5 so the building sits flush on the grid.
  const isStretchToFit =
    (source.variantType === 'standalone' ||
     source.variantType === 'standaloneConstruction' ||
     source.variantType === 'standaloneAbandoned') &&
    activePack.standaloneSrcs?.[buildingType]?.stretchToFit === true;
  const footprintN = Math.max(getBuildingSize(buildingType).width, getBuildingSize(buildingType).height);
  const destHeight = isStretchToFit
    ? footprintN * h * 2.5
    : destWidth * (coords.sh / coords.sw);
  ...
  if (isMultiTile && isStandaloneVariant) {
    // Image bottom = front tile ground → verticalPush = 0
    verticalPush = 0;
  } else if (isMultiTile) {
