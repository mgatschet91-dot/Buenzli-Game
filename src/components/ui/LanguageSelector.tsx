...
export function LanguageSelector({ 
  ...
  const handleSelectLanguage = async (code: string) => {
    ...
    if (typeof document !== 'undefined') {
      document.cookie = `generaltranslation.locale=${encodeURIComponent(code)}; path=/; max-age=31536000; samesite=lax`;
    }

    if (typeof window !== 'undefined') {
      window.localStorage.setItem('gt.locale', code);
    }

    await Promise.resolve(setLocale(code));
    setIsOpen(false);
