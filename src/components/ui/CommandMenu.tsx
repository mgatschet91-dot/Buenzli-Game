...
interface MenuItem {
  ...
  type: 'tool' | 'panel';
  tool?: Tool;
  panel?: 'gemeinde' | 'statistics' | 'advisors' | 'settings' | 'inventory' | 'navigator';
...
function buildMenuItems(): MenuItem[] {
  ...
  // Zoning category (de-zone tool only, water/land moved to terrain)
  const zoningCategory: Tool[] = ['zone_dezone'];
  zoningCategory.forEach(tool => {
    const info = TOOL_INFO[tool];
    const keywords = [info.name.toLowerCase(), tool, 'zoning', 'remove', 'clear'];
    items.push({
      id: tool,
      type: 'tool',
      tool,
  ...
  // Terrain
  const terrainCategory: Tool[] = ['terrain_raise', 'terrain_lower', 'terrain_lower2', 'terrain_hill', 'terrain_mountain', 'terrain_flatten', 'zone_water', 'zone_land'];
  terrainCategory.forEach(tool => {
    const info = TOOL_INFO[tool];
    const keywords = [info.name.toLowerCase(), tool, 'terrain', 'berg', 'hügel', 'mountain', 'hill', 'flatten'];
    if (tool === 'zone_water') {
      keywords.push('water', 'wasser', 'terraform', 'lake', 'see', 'ocean');
    } else if (tool === 'zone_land') {
      keywords.push('land', 'terraform', 'grass', 'fill', 'auffüllen');
    }
    items.push({
      id: tool,
      type: 'tool',
  ...
  // Painting
  const paintingCategory: Tool[] = ['paint_green', 'paint_sand', 'paint_dirt', 'paint_snow', 'paint_dark_grass', 'paint_rock', 'paint_reset'];
  paintingCategory.forEach(tool => {
    const info = TOOL_INFO[tool];
    items.push({
      id: tool,
      type: 'tool',
      ...
      cost: info.cost,
      category: 'painting',
      keywords: [info.name.toLowerCase(), tool, 'farbe', 'paint', 'color', 'colour'],
    });
  });
  ...
  const parksCategory: Tool[] = [
    ...
    // Parks & recreation
    'park', 'park_large', 'tennis', 'playground_small', 'playground_large', 'community_garden', 'pond_park', 'park_gate', 'greenhouse_garden', 'mini_golf_course', 'go_kart_track', 'amphitheater', 'roller_coaster_small', 'campground', 'cabin_house', 'mountain_lodge', 'mountain_trailhead', 'woodcutter_house',
  ];
  ...
  // Utilities
  const utilitiesCategory: Tool[] = ['power_plant', 'water_tower', 'subway_station', 'rail_station', 'bus_stop'];
  ...
  // Verwaltung (municipality management)
  const verwaltungCategory: Tool[] = ['bauzone', 'bauzone_remove'];
  verwaltungCategory.forEach(tool => {
    const info = TOOL_INFO[tool];
    items.push({
      id: tool,
      type: 'tool',
      ...
      cost: info.cost,
      category: 'verwaltung',
      keywords: [info.name.toLowerCase(), tool, 'verwaltung', 'bauzone', 'zone', 'bauen', 'erlaubnis', 'permission', 'admin'],
    });
  });
...
export function CommandMenu() {
  ...
  const { stats } = state;
  const m = useMessages();
  const translateLabel = useCallback((value: unknown): string => {
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        className="p-0 gap-0 max-w-lg overflow-hidden bg-slate-900/95 border-slate-700 shadow-2xl"
              ...
              MENU_CATEGORIES.map(category => {
                ...
                return (
                  <div key={category.key} className="mb-2">
                    <div className="px-2 py-1.5 text-[10px] font-bold tracking-widest text-slate-500 uppercase">
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col gap-0.5 min-w-0">
                              <span className="font-medium truncate">{translateLabel(item.name)}</span>
                              <span className={cn(
                                'text-xs truncate',
                                isSelected ? 'text-amber-200/70' : 'text-slate-400'
                              )}>
        ...
        <div className="flex items-center justify-between border-t border-slate-700 px-3 py-2 text-xs text-slate-500">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1">
              <kbd className="inline-flex h-4 items-center rounded border border-slate-700 bg-slate-800 px-1 font-mono text-[10px]">↑</kbd>
              <kbd className="inline-flex h-4 items-center rounded border border-slate-700 bg-slate-800 px-1 font-mono text-[10px]">↓</kbd>
              <span>navigate</span>
            </span>
            <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1">
            <kbd className="inline-flex h-4 items-center rounded border border-slate-700 bg-slate-800 px-1 font-mono text-[10px]">⌘</kbd>
            <kbd className="inline-flex h-4 items-center rounded border border-slate-700 bg-slate-800 px-1 font-mono text-[10px]">K</kbd>
            <span>to toggle</span>
          </span>
        </div>
      </DialogContent>
